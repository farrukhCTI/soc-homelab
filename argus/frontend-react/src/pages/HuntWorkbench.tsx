...
export default function HuntWorkbench() {
  ...
  const { huntPivot, setHuntPivot, selectedCase } = useArgus()
  ...
  useEffect(() => {
    ...
    const mergedParams = { ...defaults, ...huntPivot.params }
    setParams(mergedParams)

    // T1-3: Log HUNT_PIVOT on pivot ARRIVAL — before hunt runs, so there is always a
    // record even if the analyst abandons the hunt without executing it.
    // Filter out internal pivot meta-params (_pivotLabel, _sourceCase) from the note.
    const pivotNote = [
      `entity: ${huntPivot.label || "unknown"}`,
      `template: ${huntPivot.templateId}`,
      huntPivot.sourceCase ? `source: ${huntPivot.sourceCase}` : null,
    ].filter(Boolean).join(" · ")
    ...
    fetch("/api/actions", {
      ...
      body: JSON.stringify({
        action:      "HUNT_PIVOT",
        case_id:     huntPivot.sourceCase || selectedCase?.case_id || null,
        behavior_id: null,
        actor:       "analyst",
        note:        pivotNote,
      }),
    }).catch(() => {
      // Non-fatal — pivot navigation still proceeds even if log write fails
    })
    ...
    <div style={{ flex: 1, display: "flex", overflow: "hidden", background: "var(--bg0)" }}>
      ...
      <div style={{ width: 240, flexShrink: 0, background: "var(--bg1)", borderRight: "1px solid var(--ln)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "10px 12px 8px", borderBottom: "1px solid var(--ln)" }}>
          <span style={{ fontSize: 10, fontFamily: "var(--mono)", letterSpacing: "0.08em", color: "var(--t2)", textTransform: "uppercase" }}>Hunt templates</span>
        </div>
        <div style={{ flex: 1, overflowY: "auto" }}>
          ...
          {templates.map((t: HuntTemplate) => (
            <div key={t.id} onClick={() => selectTemplate(t)} style={{
              ...
              <div style={{ fontSize: 10, fontFamily: "var(--mono)", color: "var(--blue)", marginBottom: 2 }}>{t.id}</div>
              <div style={{ fontSize: 11, color: "var(--t1)", fontWeight: 500, marginBottom: 3 }}>{t.name}</div>
              <div style={{ fontSize: 10, color: "var(--t2)", lineHeight: 1.4 }}>{t.description}</div>
            </div>
          ))}
        </div>
      </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
            ...
            {params._pivotLabel && (
              <div style={{
                ...
                borderLeft: "3px solid var(--amb)", borderRadius: 2,
                display: "flex", alignItems: "center", gap: 8,
              }}>
                <span style={{ fontSize: 10, fontFamily: "var(--mono)", color: "var(--amb)", letterSpacing: "0.08em", textTransform: "uppercase" }}>
                  Pivot
                </span>
                ...
                {params._sourceCase && (
                  <span style={{ fontSize: 10, fontFamily: "var(--mono)", color: "var(--t2)", marginLeft: 4 }}>
            ...
            {activeTemplate.params.length > 0 && (
              <div style={{ background: "var(--bg2)", border: "1px solid var(--ln2)", borderRadius: 4, padding: "12px 16px", marginBottom: 14 }}>
                <div style={{ fontSize: 10, fontFamily: "var(--mono)", letterSpacing: "0.08em", color: "var(--t2)", textTransform: "uppercase", marginBottom: 10 }}>Parameters</div>
            ...
            {result?.query && (
              <div style={{ background: "var(--bg2)", border: "1px solid var(--ln)", borderRadius: 4, padding: "10px 14px", marginBottom: 14 }}>
                <div style={{ fontSize: 10, fontFamily: "var(--mono)", letterSpacing: "0.08em", color: "var(--t2)", textTransform: "uppercase", marginBottom: 6 }}>ES|QL Query</div>
            ...
            {result && (
              <div style={{ background: "var(--bg1)", border: "1px solid var(--ln2)", borderRadius: 4, overflow: "hidden", marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", borderBottom: "1px solid var(--ln)", background: "var(--bg2)" }}>
                  <span style={{ fontSize: 10, fontFamily: "var(--mono)", letterSpacing: "0.08em", color: "var(--t2)", textTransform: "uppercase" }}>Results</span>
                  <span style={{ fontSize: 10, fontFamily: "var(--mono)", color: "var(--t2)", background: "var(--bg3)", padding: "1px 6px", borderRadius: 2 }}>{result.total} rows</span>
                  <button onClick={askClaude} disabled={copilotLoading} style={{
                    marginLeft: "auto", fontSize: 10, fontFamily: "var(--mono)", padding: "3px 10px",
                    border: "1px solid var(--blue3)", background: "var(--blue2)", color: "var(--blue)",
                    borderRadius: 3, cursor: "pointer", opacity: copilotLoading ? 0.6 : 1,
                  }}>{copilotLoading ? "Thinking..." : "✦ Ask Claude"}</button>
                </div>

                {result.rows.length === 0 ? (
                  <div style={{ padding: 20, fontSize: 11, color: "var(--t2)", fontFamily: "var(--mono)", textAlign: "center" }}>
                        ...
                        <thead>
                          <tr style={{ background: "var(--bg2)" }}>
                            {result.columns.map((c, i) => (
                              <th key={i} style={{ padding: "6px 12px", textAlign: "left", fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", color: "var(--t2)", borderBottom: "1px solid var(--ln)", whiteSpace: "nowrap", textTransform: "uppercase" }}>
                                {c.name}
                              </th>
                            ))}
                          </tr>
            ...
            {/* Copilot */}
            {copilot && (
              <div style={{ background: "var(--bg2)", border: "1px solid var(--pur3)", borderRadius: 4, padding: "12px 16px", borderLeft: "3px solid var(--pur)" }}>
                <div style={{ fontSize: 10, fontFamily: "var(--mono)", letterSpacing: "0.08em", color: "var(--pur)", textTransform: "uppercase", marginBottom: 8 }}>Claude Co-pilot</div>
                ...
                {(copilot as any).findings?.length > 0 && <>
                  <div style={{ fontSize: 10, fontFamily: "var(--mono)", color: "var(--t2)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>Findings</div>
                  {(copilot as any).findings.map((f: string, i: number) => (
                    <div key={i} style={{ fontSize: 10, color: "var(--t2)", display: "flex", gap: 6, marginBottom: 3 }}>
                      <span style={{ color: "var(--pur)" }}>·</span><span>{f}</span>
                    </div>
                  ))}
                </>}
                {(copilot as any).recommended_actions?.length > 0 && <>
                  <div style={{ fontSize: 10, fontFamily: "var(--mono)", color: "var(--t2)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6, marginTop: 8 }}>Recommended actions</div>
                ...
                {(copilot as any).mitre_tags?.length > 0 && (
                  <div style={{ marginTop: 8, display: "flex", gap: 4, flexWrap: "wrap" }}>
                    {(copilot as any).mitre_tags.map((t: string, i: number) => (
                      <span key={i} style={{ fontSize: 10, fontFamily: "var(--mono)", padding: "2px 6px", borderRadius: 2, background: "var(--pur2)", color: "var(--pur)", border: "1px solid var(--pur3)" }}>{t}</span>
                    ))}
                  </div>
                )}
              </div>
