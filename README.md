...
- Background automation: victim VM previously ran Atomic Red Team techniques probabilistically every 30 minutes via Task Scheduler during the live build phase of this lab, see [HISTORICAL-LAB.md](docs/HISTORICAL-LAB.md)
- See [ARGUS.md](argus/ARGUS.md) for full documentation and screenshots
...
The original build forced traffic between attacker and victim through a monitored pfSense interface, ensuring all attack activity was observable by Suricata regardless of attacker behavior. This physical range has since been decommissioned. See [HISTORICAL-LAB.md](docs/HISTORICAL-LAB.md) for full details on what was built and how it operated.

- Node 1: SOC Core (Elastic SIEM + Fleet Server)
- Node 2: Proxmox lab (pfSense, Kali attacker, Windows victim), historical, no longer deployed
- pfSense: Routing + Suricata IDS + Filebeat NDR pipeline, historical
- Network segmentation used during the live build:
  - 10.0.30.0/24: Attack network (Kali)
  - 10.0.20.0/24: Victim network (Windows 10)
- Monitored traffic traversed pfSense OPT1 (Suricata interface)
...
*Figure: Segmented lab with Suricata positioned on OPT1 to monitor attack traffic between networks. This reflects the historical live range, see [HISTORICAL-LAB.md](docs/HISTORICAL-LAB.md).*

---

## Architecture Detailed

The following reflects the historical live range as it operated during the original build. It is retained as documentation and is not currently deployable from this repository.
...
The pipelines below describe how telemetry flowed during the historical live build. See [CURRENT-DEMO.md](docs/CURRENT-DEMO.md) for what runs today from this repository.
...
| Elastic Agent | 8.17.0 | EDR collection on victim, historical |
| Sysmon | v15.20 | Endpoint telemetry, historical |
| Suricata | CE (pfSense) | Network IDS, historical |
| Filebeat | 7.14.0 | NDR pipeline (pfSense FreeBSD), historical |
...
|---|---|---|---|---|
| Node 1 | Dell Inspiron 3593 | i5-1035G1 | 16GB | SOC Core, currently active |
| Node 2 | Dell E7250 | i5-5300U | 8GB | Historical Proxmox lab, decommissioned |
